...
must_haves:
  truths:
    - "ContextualSearchService searches all 8 data sources"
    - "ChromaDB semantic search respects scope via metadata where clause"
    - "Results from semantic and structured search are merged and deduplicated by (source_type, source_id)"
  artifacts:
    - path: "src/services/contextual_search_service.py"
      provides: "ContextualSearchService with scope-aware search across 8 sources"
      exports: ["ContextualSearchService", "get_contextual_search_service"]
      min_lines: 200
    - path: "src/search/vector_store.py"
      provides: "VectorStore.search_with_scope() method"
      contains: "def search_with_scope"
    - path: "tests/test_contextual_search.py"
      provides: "Integration tests for contextual search"
      min_lines: 100
  key_links:
    - from: "ContextualSearchService"
      to: "SearchContext"
      via: "constructor accepts SearchContext, uses to_chromadb_where() and to_sql_conditions()"
      pattern: "self.context"
    - from: "ContextualSearchService"
      to: "VectorStore"
      via: "_search_semantic() calls search_with_scope()"
      pattern: "search_with_scope"
    - from: "ContextualSearchService"
      to: "SiteVisitService"
      via: "extends result types SearchResponse, SiteVisitResult"
      pattern: "from src.services.site_visit_service import"
---

<objective>
Create ContextualSearchService that orchestrates scope-aware search across all 8 data sources with proper result merging

Purpose: Provide a single service that accepts a SearchContext and executes appropriately scoped searches across semantic (ChromaDB) and structured (FTS5) sources, returning merged and deduplicated results

Output: ContextualSearchService class, enhanced VectorStore with scope filtering, integration tests
</objective>
...
@.planning/phases/25-context-model-service-layer/25-01-SUMMARY.md

<interfaces>
<!-- Key types and contracts from Plan 01 -->

From src/core/models.py (created in Plan 01):
```python
...
From src/services/site_visit_service.py (existing pattern to extend):
```python
@dataclass
...
From src/search/vector_store.py (existing method):
```python
def search(
    self,
...
8 required sources (SRC-01):
1. documents (semantic via ChromaDB)
...
<task type="auto">
  <name>Task 1: Add search_with_scope method to VectorStore</name>
  <files>src/search/vector_store.py</files>
  <read_first>
    - src/search/vector_store.py (existing search() method at lines 89-142)
    - src/core/models.py (SearchContext.to_chromadb_where() method)
  </read_first>
  <action>
Add new method to VectorStore class after existing search() method (around line 143):
...
Also update add_chunks() method to include institution_id and program_id in metadata (around line 69-78):

Find the metadatas list comprehension and update it to:
```python
metadatas = [
    {
...
```
  ...
  <verify>
    <automated>grep -c "def search_with_scope" src/search/vector_store.py</automated>
  </verify>
  <acceptance_criteria>
    - grep -c "def search_with_scope" src/search/vector_store.py returns 1
    - grep -c "scope_where" src/search/vector_store.py returns at least 2
    - grep "institution_id" src/search/vector_store.py shows it in metadatas dict
    - grep "program_id" src/search/vector_store.py shows it in metadatas dict
  </acceptance_criteria>
  <done>VectorStore has search_with_scope() method that accepts scope_where dict for metadata filtering; add_chunks() includes institution_id and program_id in metadata</done>
</task>

<task type="auto" tdd="true">
  <name>Task 2: Create ContextualSearchService with 8-source search and deduplication</name>
  <files>src/services/contextual_search_service.py, tests/test_contextual_search.py</files>
  <read_first>
    - src/services/site_visit_service.py (pattern for unified search, lines 1-200)
    - src/core/models.py (SearchContext, SearchScope)
    - src/search/search_service.py (SearchService pattern)
    - src/db/migrations/0033_contextual_search.sql (FTS5 table schemas)
  </read_first>
  <behavior>
    - Test: ContextualSearchService constructor accepts SearchContext
    - Test: search() returns SearchResponse with results, total, query_time_ms, sources_searched
    - Test: _deduplicate() removes duplicates by (source_type, source_id), keeps highest score
    - Test: search() with INSTITUTION scope only returns results for that institution
    - Test: search() with GLOBAL scope returns results from all institutions
    - Test: sources_searched includes all 8 sources when all enabled
  </behavior>
  <action>
1. Create tests/test_contextual_search.py with tests for behaviors above

2. Create src/services/contextual_search_service.py:
...
```
  ...
  <verify>
    <automated>pytest tests/test_contextual_search.py -x -v</automated>
  </verify>
  <acceptance_criteria>
    - File src/services/contextual_search_service.py exists and has > 200 lines
    - grep -c "class ContextualSearchService" src/services/contextual_search_service.py returns 1
    - grep -c "ALL_SOURCES = " src/services/contextual_search_service.py returns 1
    - grep "ALL_SOURCES" src/services/contextual_search_service.py shows 8 sources in list
    - grep -c "def _deduplicate" src/services/contextual_search_service.py returns 1
    - grep -c "source_type, source_id" src/services/contextual_search_service.py returns at least 1 (dedup key)
    - grep -c "to_chromadb_where" src/services/contextual_search_service.py returns at least 1
    - pytest tests/test_contextual_search.py -x passes
  </acceptance_criteria>
  <done>ContextualSearchService searches all 8 data sources; ChromaDB semantic search uses scope_where from SearchContext; results deduplicated by (source_type, source_id) tuple; all tests pass</done>
</task>
...
1. `pytest tests/test_contextual_search.py -x` - all tests pass
2. `grep "ALL_SOURCES" src/services/contextual_search_service.py` - shows 8 sources
3. Manual verification:
   ```python
   from src.core.models import SearchContext, SearchScope
   from src.services.contextual_search_service import ContextualSearchService

   ctx = SearchContext(scope=SearchScope.INSTITUTION, institution_id="inst_123")
   svc = ContextualSearchService(ctx)
   response = svc.search("policy")
   print(response.sources_searched)  # Should show 8 sources
   ```
</verification>

<success_criteria>
1. VectorStore.search_with_scope() accepts scope_where dict and applies to ChromaDB query (SRC-02)
2. ContextualSearchService searches all 8 data sources (SRC-01)
3. Semantic search via ChromaDB respects scope via metadata filtering (SRC-02)
4. Results are deduplicated by (source_type, source_id) tuple, keeping highest score (SRC-04)
5. All tests pass
</success_criteria>

<output>
After completion, create `.planning/phases/25-context-model-service-layer/25-02-SUMMARY.md`
</output>
