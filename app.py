...
from src.api.executive import executive_bp, init_executive_bp
from src.api.onboarding import onboarding_bp, init_onboarding_bp
from src.api.consulting import consulting_bp, init_consulting_bp
...
init_executive_bp(ai_client)
init_onboarding_bp()
init_consulting_bp(workspace_manager, ai_client)
...
@app.route('/institutions/<id>/program-comparison')
def institution_program_comparison(id):
    """Cross-program comparison matrix page."""
    institution = workspace_manager.load_institution(id)
    if not institution:
        return render_template('404.html'), 404

    return render_template(
        'institutions/program_comparison.html',
        institution=institution,
        current_institution=institution,
        readiness_score=_get_readiness_score(id),
    )
